...
public class test_attch_patch_FragmentDialog extends android.support.v4.app.DialogFragment {
    ...
    public adapter_test_attch_patch_list_for_dialog adapter;

    Bundle bundle;
    String EmpID = "";

    // Empty constructor required for DialogFragment
    public test_attch_patch_FragmentDialog() {
    }

    public static test_attch_patch_FragmentDialog newInstance(String title) {
        test_attch_patch_FragmentDialog frag = new test_attch_patch_FragmentDialog();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        View view = inflater.inflate(R.layout.test_att_patch_fragment_dialog, container);
    ...
    public void onStart() {
        try {
            ...
            if (bundle != null) {
                EmpID = bundle.get("Emp_ID").toString();
                ...
                lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
                                              ...
                                              public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                                                  try {
                                                      ...
                                                      sendBackTestPatchResult(clickedObj.getName(), clickedObj.getPatch_name());
                                                  } catch (Exception ex) {
    ...
    public void Bind_data_listview() {
        try {
            final RealmResults<POJO_Patch_master_S> result_query1 = mRealm.where(POJO_Patch_master_S.class).findAll().sort("patch_name", Sort.ASCENDING);
            List<POJO_Patch_master_S> mList = result_query1;

            adapter = new adapter_test_attch_patch_list_for_dialog(getContext(), R.layout.adapter_test_att_patch_list_for_dialog, mList);
    ...
    public interface EditTestPacthDialogListener {
        void onFinishTestPacthDialog(String id, String fullname);
    }

    // Call this method to send the data back to the parent fragment
    public void sendBackTestPatchResult(String id, String name) {
        try {
            // Notice the use of `getTargetFragment` which will be set when the dialog is displayed
            EditTestPacthDialogListener listener = (EditTestPacthDialogListener) getTargetFragment();
            listener.onFinishTestPacthDialog(id, name);
            dismiss();
        } catch (Exception ex) {
            context = getActivity();
            if(context!=null) {
    ...
    public void Load_User() {
        try {
            ...
            restService.getService().getPatchUserWise(sid, "modified desc", limitstart, pagesize, jsonArray, Filters, new Callback<JsonElement>() {
                @Override
